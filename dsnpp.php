...
<!-- !PAGE CONTENT! -->
 ...
 <div class="w3-section w3-yellow w3-padding-10"> <!-- thanh ngang -->
    ...
    <h1 style="margin-top: 10px;"><b>QUẢN LÝ NHÀ PHÂN PHỐI</b></h1>
    </div>
    </div>
  </header>
...
// Xóa nhà phân phối nếu có yêu cầu
if (isset($_GET['delete_id'])) {
    $distributor_id = $_GET['delete_id'];
    $sql_delete = "DELETE FROM distributors WHERE distributor_id = $distributor_id";

    if ($conn->query($sql_delete) === TRUE) {
        echo "<script>alert('Xóa nhà phân phối thành công'); window.location.href='dsnpp.php';</script>";
    } else {
        echo "<div class='alert alert-danger'>Lỗi: " . $conn->error . "</div>";
    }
}

// Truy vấn danh sách nhà phân phối
        ...
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                echo "<tr>
                        ...
                        <td>
                            <a href='npp_phieu.php?distributor_id=" . $row['distributor_id'] . "' class='btn btn-info btn-primary btn-sm'>Xem phiếu nhập</a><br><br>
                            <a href='upnpp.php?distributor_id=" . $row['distributor_id'] . "' class='btn btn-warning'>Chỉnh sửa</a><br><br>
                            <a href='dsnpp.php?delete_id=" . $row['distributor_id'] . "' class='btn btn-danger' onclick='return confirm(\"Bạn có chắc chắn muốn xóa nhà phân phối này?\");'>Xóa</a>
                        </td>
                      </tr>";
            }
        } else {
            echo "<tr><td colspan='6'>Không có nhà phân phối nào</td></tr>";
        }
        ?>
        </tbody>
    </table>
     <a href="themnpp.php" onclick="close()" style="margin-left: 10px; margin-top: 10px; color: black;" class="w3-bar-item w3-button w3-padding"><i class="fa fa-plus fa-fw w3-margin-right"></i><strong>Thêm nhà phân phối</strong></a> 
</div>
</body>
</html>
<?php
