...
// }
  ...
  if(isset($_POST['distributors'])){
         ...
         if($conn ->query(
                           "INSERT INTO distributors (distributor_name, address, bank_account_number, phone_number) VALUES ('$distributor_name', '$address', '$bank_account_number', '$phone_number')"   
                         )
            )
                    {
...
<!-- !PAGE CONTENT! -->
 ...
 <!-- form -->
  <div class="w3-container w3-padding-large w3-white" style="margin-top: 50px;"> 
    <h4 class="w3-center" style="font-size: 25pt; margin-top: 40px; margin-bottom:-65px;" id="lienhe"><b>Thêm nhà phân phối</b></h4>
    <form action="connecttnpp.php" method="post">
      <!-- <b><label>ID: </label></b> <input type="hidden" id="id"  size="5" name="id"> -->
    <b><label>Mã nhà phân phối: </label></b> <input type="distributor_id" id="distributor_id"  size="5" name="distributor_id">
    <div class="w3-row-padding w3-center w3-padding-24" style="margin:0 -16px"></div>
    
    <hr class="w3-opacity">
    <!-- <form action="/action_page.php" target="_blank"> -->
      <div class="w3-section">
        <label><b>Tên nhà phân phối:</b></label>
        <input type="distributor_name" style="block-size: 40px;" class="w3-input w3-border" required="" name="distributor_name">
      </div>
      <div class="w3-section">
      ...
      <button type="submit" name="distributors" class="w3-button w3-green w3-margin-bottom"><i style="color: white;" class="fa fa-paper-plane w3-margin-right"></i>Thêm nhà phân phối</button>
    </form>
  </div>
